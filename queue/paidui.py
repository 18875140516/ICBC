import sys
from yolo import YOLO
import cv2
import json
import os
from PIL import Image
from utils.util import checkPoint
import argparse
import base64

from configRetrive import ConfigRetrive
from rtmpAgent import RTMP_AGENT
OK = True
paidui_config = ConfigRetrive()
USE_WS = True
USE_FFMPEG = False
USE_IMSHOW = False

MQTT_URL = '211.67.21.65'
if USE_WS:
    import paho.mqtt.publish as publish
def main(yolo, cfg):
    cap = cv2.VideoCapture(cfg['VIDEO_PATH'])
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    if USE_FFMPEG:
        agent = RTMP_AGENT(sizeStr=str(width)+'x' +str(height), topic='paidui')
    DEAULT_AREA = cfg['DEAULT_AREA']

    # def onMousecb(event, x, y, flags, params):
    #     if event == cv2.EVENT_LBUTTONDOWN:
    #         print(checkPoint((x, y), points))
    if USE_IMSHOW:
        cv2.namedWindow('win')
        # cv2.setMouseCallback('win', onMousecb)
    idx = 0
    while OK:
        ret, frame = cap.read()
        if ret == False:
            cap = cv2.VideoCapture(cfg['VIDEO_PATH'])
            ret, frame = cap.read()
        image = Image.fromarray(frame[..., ::-1])  # bgr to rgb
        boxs, classname = yolo.detect_image(image)  # xy w h
        cp = frame.copy()
        queueSize = 0
        points = paidui_config.get(key='entry', default_value=DEAULT_AREA)
        points = [[int(x * width), int(y * height)] for [x, y] in points]
        for box in boxs:
            cpt = ((box[0]*2 + box[2])/2, box[1]+box[3])
            if checkPoint(cpt, points):
                cv2.rectangle(cp, (box[0], box[1]), (box[0]+box[2], box[1]+box[3]), (0,0,255), 2)
                queueSize += 1
            else:
                cv2.rectangle(cp, (box[0], box[1]), (box[0] + box[2], box[1] + box[3]), (255, 255, 255), 2)
        for i in range(len(points)):
            cv2.line(cp, tuple(points[i]), tuple(points[(i+1)%len(points)]), (0, 0, 255), thickness=2)
        if queueSize > int(paidui_config.get('waitNumber', 10)):
            publish.single('warning', payload='too much people are waiting!', hostname=MQTT_URL)
            cv2.putText(cp, 'too much people', (200, 50), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255),
                        thickness=2)
        cv2.putText(cp,'queue: '+str(queueSize), (50,50), cv2.FONT_HERSHEY_PLAIN, 2, (255,255,255), thickness=2)
        if USE_IMSHOW:
            cv2.imshow('win', cp)
            q = cv2.waitKey(30)
            if q == ord('q'):
                break
            elif q == ord('x'):
                cv2.imwrite('{:04d}.jpg'.format(idx), cp)
        elif USE_WS:
            s = base64.b64encode(cv2.imencode('.jpg', cp)[1])
            publish.single(topic='offlineImage', payload=s, hostname=MQTT_URL)
        elif USE_FFMPEG:
            agent.send_image(cp)
        idx += 1
    pass


if __name__ == '__main__':
    with open('../config/init.json', 'r') as f:
        cfg = json.load(f)
    parser = argparse.ArgumentParser()
    parser.add_argument('--model', type=str, default=os.path.abspath('../model_data/yolo.h5'))
    parser.add_argument('--classpath', type=str, default=os.path.abspath('../model_data/coco_classes.txt'))
    parser.add_argument('--anchorpath', type=str, default=os.path.abspath('../model_data/yolo_anchors.txt'))
    args = parser.parse_args()
    main(YOLO(model_path=args.model,class_path=args.classpath, anchor_path=args.anchorpath), cfg)
