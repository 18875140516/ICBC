...
publish_client = client.Client()
publish_client.connect(host=MQTT_URL)
...
def main(yolo, args):  # 输入yolov3模型和视频路径
    ...
    # topic = 'standing_area'
    # publish_client.publish(topic='config',payload=json.dumps({topic:img_b64}))
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            mostStandingTime = max(mostStandingTime, track.standing_time())
        ...
        #transfer to minute
        mostStandingTime = mostStandingTime//60
        ...
        if queueSize > int(online_config.get('waitNumber', 999)):
            ...
            publish_client.publish('warning', payload=json.dumps(root))
        if mostStandingTime > int(online_config.get('waitTime', '999')):
        ...
        publish_client.publish('numQueue', payload=json.dumps(num_json))

        standing_json['mostStandingTime'] = str(mostStandingTime)
        publish_client.publish('mostStandingTime', payload=json.dumps(standing_json))
        ...
        if USE_IMSHOW:
            ...
            publish_client.publish(topic='offlineImage', payload=s)
        elif USE_FFMPEG:
